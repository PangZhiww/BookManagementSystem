...
def login(request):
    ...
    # 如果你是第一次来，是跟我偶要一个登录页面来填写数据的    --> GET
...
def press_list(request):
    # 1. 去数据库查所有的出版社
    ret = Press.objects.all().order_by('id')  # order_by排序
    print(ret)
    # print(ret[0])
    # print(ret[0].name)
    # 2. 在HTML页面上展示出来
...
def xiaojilei(request):
    ...
    # 1. 返回一个页面，让用户填写书籍信息
    # 因为书籍信息需要关联出版社
    # 所以在添加书籍的页面要把已经存在的出版社展示出来，让用户选择
...
def author_list(request):
    ...
    for author in author_data:
        ...
        print(author.books.all())  # 是一个ORM提供的桥梁（工具），帮我找对应关系
    # 2. 在页面上展示出来
    return render(request, 'author_list2.html', {'author_list': author_data})
...
def add_author(request):
    ...
    return render(request, 'add_author2.html', {'book_list': book_data})
...
def edit_author(request):
    ...
    book_data = Book.objects.all()
    # 3. 返回一个页面
    return render(request, 'edit_author2.html', {'author': edit_author_obj, 'book_list': book_data})
